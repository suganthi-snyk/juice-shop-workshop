        ...
        const imageRequest = request
          ...
          .on('response', function (res: Response) {
            if (res.statusCode === 200) {
              const extRaw = url.split('.').slice(-1)[0]?.toLowerCase() ?? 'jpg'
              const allowedExt = ['jpg', 'jpeg', 'png', 'svg', 'gif']
              const ext = allowedExt.includes(extRaw) ? extRaw : 'jpg'
              const uploadsBase = 'frontend/dist/frontend/assets/public/images/uploads'
              const safePath = utils.resolvePathUnder(uploadsBase, `${loggedInUser.data.id}.${ext}`)
              if (!safePath) {
                UserModel.findByPk(loggedInUser.data.id).then(async (user: UserModel | null) => { return await user?.update({ profileImage: url }) }).catch((error: Error) => { next(error) })
                return
              }
